...
import { PendingRegistration, Business, Shop, Warehouse, User } from "@/models";
...
export async function initiateRegistration(
  ...
  try {
    ...
    await PendingRegistration.update({ step: 2 }, { where: { id: registrationId } });
...
export async function getRegistrationStatus(
  registrationId: string
): Promise<{ success: boolean; data?: Partial<PendingRegistration>; error?: string }> {
  try {
    const registration = await PendingRegistration.findByPk(registrationId, {
      attributes: ["id", "email", "firstName", "lastName", "businessName", "step", "otpVerified"],
    });
...
export async function completeRegistration(
  ...
  try {
    const registration = await PendingRegistration.findByPk(data.registrationId);
    ...
    if (!registration.otpVerified) {
      await transaction.rollback();
      return { success: false, error: "Email not verified. Please complete OTP verification first." };
    }
    ...
    const business = await Business.create(
      {
        ...
        registrationNumber: registration.businessRegNumber,
        email: registration.email,
    ...
    // Create Shop (auto-named)
    await Shop.create(
      {
        businessId: business.id,
        name: `${registration.businessName} Store`,
        slug: generateSlug(registration.businessName),
        isActive: false,
      },
      { transaction }
    );
    ...
    await Warehouse.create(
      {
        businessId: business.id,
        name: `${registration.businessName} Main Warehouse`,
        address: registration.businessAddress,
        isActive: false,
      },
      { transaction }
